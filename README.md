<div align="center">

# 🚀 **MIT Learning Path – Gen Meca**
### *A Professional, MIT-Aligned Engineering Roadmap*

![Banner](https://img.shields.io/badge/Gen%20Meca-MIT%20Learning%20Path-blue?style=for-the-badge)

---

### 🔧 **Mechatronics | Robotics | Control | Materials | Programming**
A structured and research-oriented self-study program following MIT OpenCourseWare.

</div>

---

# 📑 **Table of Contents**
- [🎯 Vision](#-vision)
- [📘 Purpose](#-purpose)
- [📂 Directory Tree](#-directory-tree)
- [📊 Progress Tracking](#-progress-tracking)
- [📚 Course Folder Structure](#-course-folder-structure)
- [🛠 Workflow & Tools](#-workflow--tools)
- [👤 Author – Gen Meca](#-author--gen-meca)
- [⭐ Final Goal](#-final-goal)

---

# 🎯 Vision
Build a **world-class engineering foundation** across four core domains:

- **Mechanics**
- **Materials**
- **Programming**
- **Control Systems**

This roadmap supports my long-term plan to specialize in **advanced humanoid robotics**, forming part of my **12-domain engineering ecosystem**.

---

# 📘 Purpose
This repository is designed to be:

- A **MIT-standard learning portfolio**
- A **transparent academic record** (assignments, notes, simulations)
- A **professional profile** for lab applications (HCMUT + international)
- A **research-ready base** for robotics projects

---

# 📂 Directory Tree

```bash
MIT-Learning-Path/
│
├── Mechanics/
│   ├── Foundation/
│   │   ├── 2.001-Mechanics-and-Materials-I/
│   │   ├── 2.002-Mechanics-and-Materials-II/
│   │   ├── 2.003-Dynamics-and-Control-I/
│   │   ├── 2.004-Dynamics-and-Control-II/
│   │   ├── 2.06-Fluid-Dynamics/
│   │   ├── 2.007-Design-and-Manufacturing-I/
│   │   └── 2.008-Design-and-Manufacturing-II/
│   │
│   └── Advanced/
│       ├── 2.75-Precision-Machine-Design/
│       └── 2.092-FEA-of-Solids-and-Fluids/
│
├── Materials/
│   ├── Foundation/
│   │   ├── 3.20-Materials-At-Equilibrium/
│   │   ├── 3.032-Mechanical-Behavior-Of-Materials/
│   │   ├── 3.044-Materials-Processing/
│   │   ├── 3.054-Cellular-Solids/
│   │   ├── 3.064-Polymer-Engineering/
│   │   └── 3.091SC-Intro-To-Solid-State-Chemistry/
│   │
│   └── Advanced/
│       ├── 3.45-Magnetic-Materials/
│       ├── 3.052-Nanomechanics-of-Materials/
│       └── 3.60-Symmetry-Structure-Tensor-Properties/
│
├── Programming/
│   ├── Foundation/
│   │   ├── 6.0001-Intro-To-CS-and-Python/
│   │   ├── 6.0002-Computational-Thinking-and-Data-Science/
│   │   ├── 6.006-Introduction-To-Algorithms/
│   │   ├── 6.033-Computer-Systems-Engineering/
│   │   ├── 6.034-Artificial-Intelligence/
│   │   └── 6.172-Performance-Engineering-of-Software/
│   │
│   └── Advanced/
│       ├── 6.004-Computation-Structures/
│       ├── 6.046J-Design-and-Analysis-of-Algorithms/
│       └── 6.336J-Intro-To-Numerical-Simulation/
│
└── Control/
    ├── Foundation/
    │   ├── 2.04A-Systems-and-Controls/
    │   ├── 2.12-Introduction-To-Robotics/
    │   ├── 2.14-Feedback-Control-Systems-Design/
    │   ├── 6.003-Signals-and-Systems/
    │   ├── 6.302-Feedback-Systems/
    │   ├── 6.832-Underactuated-Robotics/
    │   └── 6.4210-Robotic-Manipulation/
    │
    └── Advanced/
        ├── 6.231-Dynamic-Programming-and-Stochastic-Control/
        ├── 6.245-Multivariable-Control-Systems/
        └── 6.441-Information-Theory/
```
## 📊 Progress Tracking

### **Mechanics**
- [ ] 2.001 Mechanics and Materials I  
- [ ] 2.002 Mechanics and Materials II  
- [ ] 2.003 Dynamics and Control I  
- [ ] 2.004 Dynamics and Control II  
- [ ] 2.06 Fluid Dynamics  
- [ ] 2.007 Design and Manufacturing I  
- [ ] 2.008 Design and Manufacturing II  
- [ ] 2.75 Precision Machine Design  
- [ ] 2.092 Finite Element Analysis  

### **Materials**
- [ ] 3.20 Materials at Equilibrium  
- [ ] 3.032 Mechanical Behavior of Materials  
- [ ] 3.044 Materials Processing  
- [ ] 3.054 Cellular Solids  
- [ ] 3.064 Polymer Engineering  
- [ ] 3.091SC Intro to Solid State Chemistry  
- [ ] 3.45 Magnetic Materials  
- [ ] 3.052 Nanomechanics of Materials  
- [ ] 3.60 Symmetry, Structure, and Tensor Properties  

### **Programming**
- [ ] 6.0001 Intro to CS and Python  
- [ ] 6.0002 Computational Thinking  
- [ ] 6.006 Algorithms  
- [ ] 6.033 Systems Engineering  
- [ ] 6.034 Artificial Intelligence  
- [ ] 6.172 Performance Engineering  
- [ ] 6.004 Computation Structures  
- [ ] 6.046J Design & Analysis of Algorithms  
- [ ] 6.336J Numerical Simulation  

### **Control**
- [ ] 2.04A Systems & Controls  
- [ ] 2.12 Introduction to Robotics  
- [ ] 2.14 Feedback Control  
- [ ] 6.003 Signals & Systems  
- [ ] 6.302 Feedback Systems  
- [ ] 6.832 Underactuated Robotics  
- [ ] 6.4210 Robotic Manipulation  
- [ ] 6.231 Dynamic Programming  
- [ ] 6.245 Multivariable Control  
- [ ] 6.441 Information Theory  


---

## 📚 Course Folder Structure
```bash
📁 Course-Name
├── 📝 Notes
├── 📚 Assignments
├── 🧪 Projects
├── 🔗 Resources
└── 📄 README.md
```

This structure mirrors real MIT OpenCourseWare academic repositories.

---

## 🛠 Workflow & Tools

### **Core Tools**
- **GitHub** – version control & academic portfolio  
- **VS Code** – coding + note-taking  
- **Python, MATLAB, Simulink** – algorithms, simulation, control modeling  
- **CAD tools (Fusion 360 / SolidWorks)** – mechanical & materials design  
- **Markdown** – clean engineering documentation  

### **Study Philosophy**
- **Depth > Speed** – prioritize mastery  
- **Assignments > Videos** – real engineering skill  
- **Everything connected to robotics**  
- **Clear documentation = academic credibility**  

---

## 👤 Author – *Gen Meca*

**Mechatronics Engineering – PFIEV / INSA pathway**

Focused on **12 long-term engineering domains**, including:  
**Mechanics • Materials • CAD • Programming • Control • Soft Robotics**  
(+ 6 supporting domains)

Building a complete, multi-disciplinary ecosystem for:

🔧 **Humanoid robotic arm development**  
🤖 **Advanced robotics research**  
📚 **Academic / international opportunities**

---

## ⭐ Final Goal

A transparent, academically credible, MIT-aligned portfolio strong enough for:

- 🔬 **University laboratory applications**  
- 🤝 **Research internships**  
- 🤖 **Robotics collaborations**  
- 🌍 **International academic opportunities**

---

<div align="center">

## 🚀 **Keep Building. Keep Learning.  
The future starts here.**

</div>


